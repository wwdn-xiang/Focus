...
def xception(inputs, num_classes=1001, is_training=True, scope="Xception"):
    '''
    The Xception model
    :param inputs: a 4D Tensor input of shape [batch_size, height, width, num_channels]
    :param num_classes: the number of classes to predict
    :param istraining: Whether or not to train
    :param scope:
    :return:
...
　　
    ...
    with slim.arg_scope([slim.separable_conv2d], depth_multiplier=1), \
        ...
        net = slim.batch_norm(net, scope='block1_relu2')
        ...
        net = slim.max_pool2d(net, [3, 3], stride=2, padding='same', scope='block3_max_pool')
        net = tf.add(net, residual, name='block4_add')  # 19*19*728 feature maps

        # ============MIDDLE FLOW============

        for i in range(8):
            block_prefix = 'block%s_' % (i + 5)
            residual = net
